...
const Register = () => {
  ...
  const [passwordsMatchValid, setPasswordsMatchValid] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValid || !passwordValid || !passwordsMatch) {
      return;
    }
    ...
    try {
      ...
      if (apiError?.errors && typeof apiError.errors === 'object') {
        ...
        setFieldErrors({});
        setErrorMessage(apiError?.message || err.message || 'Registration failed. Please check your information and try again.');
      }
