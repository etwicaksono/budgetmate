...
import { Container, Row, Col, Card, Button, Form, Offcanvas, InputGroup, Dropdown } from 'react-bootstrap';
...
// Local SortDropdown component (was missing file)
const SORT_OPTIONS = [
  { value: 'timeAsc', label: 'Time (oldest first)' },
  ...
  { value: 'absAmountAsc', label: 'Absolute Amount (lowest first)' },
  { value: 'absAmountDesc', label: 'Absolute Amount (highest first)' },
];
    ...
    >
      ...
      >
        <span className="d-flex align-items-center gap-2">
          <span className="text-truncate">{selectedOption?.label || 'Select sort order'}</span>
        </span>
      </Dropdown.Toggle>
...
function TransactionsContent() {
  ...
  const [setLastTransaction] = useState(null);
  ...
  useEffect(() => {
    if (location.state?.openTransactionModal) {
      ...
      delete restState.openTransactionModal;
  ...
  const [sortOption, setSortOption] = useState('amountAsc');
  ...
  const totalAmount = useMemo(
    () => filteredTransactions.reduce((accumulator, transaction) => accumulator + transaction.amount, 0),
    [filteredTransactions]
  );
  ...
  const formatTransactionDate = useCallback((dateValue, dateTimeValue) => {
  ...
  const handleToggleTransactionSelect = useCallback((transactionId) => {
    setSelectedTransactionIds((previous) => {
  ...
  const getSurfaceColor = useCallback((hexColor, alpha = 0.12) => {
  ...
  const resolveTagTone = useCallback((label) => {
    if (!label) {
      return 'neutral';
    }
  ...
  const handleSaveTransaction = (createAnother = false) => {
  ...
  const toggleFilterSidebar = () => setShowFilterSidebar((previous) => !previous);
              ...
              <div className="transactions-ledger__list">
                {filteredTransactions.length === 0 ? (
                  ...
                  filteredTransactions.map((transaction) => {
                    const CategoryIcon = categoryIcons[transaction.category] || FaTags;
                    const categoryColor =
                    ...
                    } else if (Array.isArray(transaction.labels)) {
                      transaction.labels.filter(Boolean).forEach((label) => labelEntries.push(label));
                    }
