...
const CashFlowReport: React.FC<CashFlowReportProps> = ({
  ...
  const comparisonChartData = useMemo(() => {
    if (!data) return [];
    const comparison = data.comparisonData[comparisonTab];
  ...
  const isPositive = data.summary.percentChange >= 0;
  const maxValue = Math.max(data.summary.totalIncome, data.summary.totalExpense);
  const incomePercent = maxValue > 0 ? (data.summary.totalIncome / maxValue) * 100 : 0;
  const expensePercent = maxValue > 0 ? (data.summary.totalExpense / maxValue) * 100 : 0;
        ...
        <Card.Body>
          ...
          <div style={{ height: 300 }}>
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={data.dailyData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
                ...
                <Line type="monotone" dataKey="cashFlow" stroke="#1f2937" strokeWidth={2} dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </Card.Body>
      ...
      <div className="mb-4">
          ...
          {data.summary.percentChange !== 0 && (
            <div className="text-end">
              ...
              >
                ...
                >
                  {Math.abs(data.summary.percentChange)}%
                </span>
              </div>
            </div>
          )}
        ...
        <div className="mb-3">
          <div className="d-flex justify-content-between align-items-center mb-1">
            ...
            <span style={{ fontWeight: 500 }}>{formatCurrency(data.summary.totalIncome)}</span>
          </div>
          <div className="rounded" style={{ height: 8, backgroundColor: '#e9ecef', overflow: 'hidden' }}>
            <div className="h-100 rounded" style={{ width: `${incomePercent}%`, backgroundColor: '#22c55e' }} />
          </div>
        </div>
        ...
        <div className="mb-3">
          <div className="d-flex justify-content-between align-items-center mb-1">
            ...
            <span style={{ fontWeight: 500 }}>-{formatCurrency(data.summary.totalExpense)}</span>
          </div>
          <div className="rounded" style={{ height: 8, backgroundColor: '#e9ecef', overflow: 'hidden' }}>
            <div className="h-100 rounded" style={{ width: `${expensePercent}%`, backgroundColor: '#ef4444' }} />
          </div>
        </div>
      </div>
      ...
      <Card className="border-0 shadow-sm">
        <Card.Body>
          <Nav variant="pills" className="comparison-tabs gap-1 justify-content-center mb-3">
            <Nav.Item>
              <Nav.Link className={comparisonTab === 'cashFlow' ? 'active' : ''} onClick={() => setComparisonTab('cashFlow')}>
                Cash flow
              </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link className={comparisonTab === 'expense' ? 'active' : ''} onClick={() => setComparisonTab('expense')}>
                Expense
              </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link className={comparisonTab === 'income' ? 'active' : ''} onClick={() => setComparisonTab('income')}>
                Income
              </Nav.Link>
            </Nav.Item>
          </Nav>
          ...
          <div style={{ height: 350 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={comparisonChartData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
                ...
                <Line type="monotone" dataKey="previous" stroke="#22c55e" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="yearAgo" stroke="#f97316" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card.Body>
