...
export const PeriodNavigationProvider: React.FC<PeriodNavigationProviderProps> = ({
  ...
  const handleSelectPreset = useCallback(
    (presetKey: string) => {
      ...
      const meta: ActivePeriodMeta = { type: preset.meta.type };
      if (preset.meta.year !== undefined) {
        meta.year = preset.meta.year;
      }
      if (preset.meta.month !== undefined) {
        meta.month = preset.meta.month;
      }
      applyRange(preset.range, preset.label, meta);
    },
    [applyRange]
  );
  ...
  const handleSelectYear = useCallback(
    (year: number) => {
      ...
      applyRange(range, `Year ${year.toString()}`, { type: 'year', year });
    },
    [applyRange]
  );

  const handleCustomDraftChange = useCallback((range: Partial<PeriodRange>) => {
    setCustomRangeDraft({
      start: range.start ?? '',
      end: range.end ?? '',
    });
